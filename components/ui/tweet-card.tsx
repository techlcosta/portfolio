...
export const TweetSkeleton = ({ className, ...props }: { className?: string; [key: string]: unknown }) => (
  <div className={cn('flex size-full max-h-max min-w-72 flex-col gap-2 rounded-xl border p-4', className)} {...props}>
    ...
    <div className="flex items-center space-x-3">
      ...
      <div className="flex flex-col gap-0.5">
        ...
        <div className="flex items-center space-x-1">
          <a href={tweet.user.url} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-foreground text-sm transition-colors">
  ...
  <div className="text-[15px] leading-relaxed tracking-normal wrap-break-word">
    {tweet.entities.map((entity, idx) => {
      switch (entity.type) {
        ...
        case 'text':
          return <span key={idx} className="text-foreground text-[15px] font-normal" dangerouslySetInnerHTML={{ __html: entity.text }} />
        default:
          return null
      }
    })}
...
export const MagicTweet = ({ tweet, className, ...props }: { tweet: Tweet; className?: string }) => {
  ...
  return (
    <div className={cn('relative flex h-fit w-full max-w-lg flex-col gap-4 overflow-hidden rounded-xl border p-5', className)} {...props}>
